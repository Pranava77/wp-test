...
function webfixus_setup() {
	...
	// logo is uploaded it replaces the "WEBFIXUS" wordmark in the header/footer;
...
function webfixus_assets() {
	...
	$css_path = get_template_directory() . '/assets/css/webfixus.css';
	$css_ver  = file_exists( $css_path ) ? filemtime( $css_path ) : WEBFIXUS_VERSION;
	wp_enqueue_style(
		'webfixus-style',
	...
	$js_path = get_template_directory() . '/assets/js/webfixus-anim.js';
	$js_ver  = file_exists( $js_path ) ? filemtime( $js_path ) : WEBFIXUS_VERSION;
	wp_enqueue_script(
		'webfixus-anim',
		get_template_directory_uri() . '/assets/js/webfixus-anim.js',
		array(),
		$js_ver,
		true
	);
